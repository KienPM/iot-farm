...
class NganLuongCheckout
{
    ...
    protected $nganluong_url;
    // Email tài khoản Ngân Lượng
    protected $receiver;
    // Mã website của bạn đăng ký trong chức năng tích hợp thanh toán của NgânLượng.vn.
    protected $merchant_site_code; //100001 chỉ là ví dụ, bạn hãy thay bằng mã của bạn
    // Mật khẩu giao tiếp giữa website của bạn và NgânLượng.vn.
    protected $secure_pass; //d685739bf1 chỉ là ví dụ, bạn hãy thay bằng mật khẩu của bạn
    ...
    protected $affiliate_code; //Mã đối tác tham gia chương trình liên kết của NgânLượng.vn


    public function __construct()
    {
        $this->nganluong_url = config('services.nganluong.url');
        $this->receiver = config('services.nganluong.receiver');
        $this->merchant_site_code = config('services.nganluong.merchant_id');
        $this->secure_pass = config('services.nganluong.merchant_pass');
        $this->affiliate_code = config('services.nganluong.affiliate_code');
    }
    ...
    public function buildCheckoutUrlExpand(
        ...
        if ($receiver == "") $receiver = $this->receiver;
        $arr_param = array(
